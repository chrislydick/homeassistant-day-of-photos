#!/usr/bin/env python3
"""
Enhanced OneDrive Photos Script with Better Token Management
...
class EnhancedOneDrivePhotosFetcher:
    def __init__(
        self,
        client_id: str,
        client_secret: str,
        ...
        photos_folder: str = "Pictures/Camera Roll"
    ):
            ...
            try:
                ...
                if token_data and 'access_token' in token_data:
                    ...
                    if self._test_token():
                        ...
                        logger.warning("⚠️ Token is invalid or expired, will re-authenticate")
                        return False
                else:
    ...
    def _refresh_token(self) -> bool:
        ...
        if not self.refresh_token:
            logger.info("ℹ️ No refresh token available, will re-authenticate")
            return False
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            print("=" * 50)
            print("Your token has expired. Please authenticate:")
            print("=" * 80)
            print(auth_url)
            print("=" * 80)
    ...
    def _get_photo_date(self, file_item) -> Optional[datetime]:
        ...
        try:
            ...
            patterns = [
                ...
                r"^(\d{4})(\d{2})(\d{2})_",  # YYYYMMDD_ (your format)
            ]
            ...
            # Method 2: Try to get date from file metadata
            if 'fileSystemInfo' in file_item:
                created = file_item['fileSystemInfo'].get('createdDateTime')
                if created:
                    return datetime.fromisoformat(created.replace('Z', '+00:00'))
            
            # Method 3: Try to get date from photo metadata
            if 'photo' in file_item:
                taken = file_item['photo'].get('takenDateTime')
                if taken:
                    return datetime.fromisoformat(taken.replace('Z', '+00:00'))
            
            return None
            
        except Exception as e:
            logger.warning(f"Could not extract date from {file_item.get('name', 'unknown')}: {e}")
            return None
    ...
    def download_photos(self, photos_data: List[Dict[str, Any]], skip_existing: bool = True) -> List[Path]:
            ...
            try:
                ...
                if file_extension == '.heic':
                    ...
                    if jpg_path:
                        ...
                        logger.warning(f"⚠️ HEIC conversion failed, keeping original: {filename}")
                        downloaded_files.append(file_path)
                else:
                    downloaded_files.append(file_path)
    ...
    def fetch_and_download(
        ...
        if not photos_data:
            logger.info("ℹ️ No photos found")
            return []
...
def main():
    """Main function with enhanced token management."""
    parser = argparse.ArgumentParser(description="Enhanced OneDrive Photos Fetcher")
    ...
    parser.add_argument("--day-range", type=int, default=1, help="Day range to search (± days)")
    parser.add_argument("--output-dir", default="./images", help="Output directory for downloaded photos")
    ...
    parser.add_argument("--photos-folder", default="Pictures/Camera Roll", help="OneDrive photos folder name")
    ...
    if years_back == 10:  # If using default, check for env var
        ...
        if env_years_back:
            try:
                ...
                logger.warning(f"⚠️ Invalid ONEDRIVE_YEARS_BACK value: {env_years_back}, using default")
    ...
    if day_range == 1:  # If using default, check for env var
        ...
        if env_day_range:
            try:
                ...
                logger.warning(f"⚠️ Invalid ONEDRIVE_DAY_RANGE value: {env_day_range}, using default")
    ...
    logger.info(f"🚀 Starting Enhanced OneDrive photos script for {target_date.date()}")
    
    try:
        # Create fetcher and download photos
        fetcher = EnhancedOneDrivePhotosFetcher(
            client_id=client_id,
        ...
        if not args.skip_transfer and downloaded_files:
            ...
            logger.info("ℹ️ Skipping transfer to Home Assistant as requested")
        else:
